...
handled by a user manager implementing ``UserBundle\Model\UserManagerInterface``.
...
of ``UserBundle\Doctrine\UserManager``.

If you configure the ``db_driver`` option to ``mongodb``, this service is an
instance of ``UserBundle\Doctrine\UserManager``.
...
a service implementing ``UserBundle\Model\UserManagerInterface`` and
...
Your custom implementation can extend ``UserBundle\Model\UserManager``
...
versions. Use ``UserBundle\Security\UserProvider`` instead.
